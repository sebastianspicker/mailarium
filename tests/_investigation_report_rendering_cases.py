...
_SAMPLE_FINDING_EVIDENCE_INDEX = {
    "findings": [
        {
...
_SAMPLE_MULTI_SOURCE_CASE_BUNDLE = {
...
def _assert_executive_summary(report):
...
def _assert_evidence_triage(report):
...
def _assert_legal_outputs(report):
...
def _assert_actors_and_witnesses(report):
...
def _assert_skeptical_and_checklists(report):
...
def _assert_matter_evidence_index(report):
...
def _assert_language_and_overall(report):
...
def test_build_investigation_report_renders_supported_sections_with_evidence_links():
    report = build_investigation_report(
        ...
        actor_identity_graph=_SAMPLE_ACTOR_IDENTITY_GRAPH,
        finding_evidence_index=_SAMPLE_FINDING_EVIDENCE_INDEX,
        evidence_table={
            "rows": [
                {
                    "finding_id": "message:uid-1:authored:escalation:1",
        ...
        multi_source_case_bundle=_SAMPLE_MULTI_SOURCE_CASE_BUNDLE,
    )

    assert report is not None
    _assert_report_top_level(report)
    _assert_executive_summary(report)
    _assert_evidence_triage(report)
    _assert_matter_evidence_index(report)
    _assert_legal_outputs(report)
    _assert_actors_and_witnesses(report)
