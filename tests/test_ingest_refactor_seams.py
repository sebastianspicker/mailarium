...
def test_ingest_wrapper_delegates_to_pipeline_module() -> None:
    with patch("mailarium.ingest_pipeline.ingest_impl", return_value={"emails_parsed": 3}) as mock_impl:
        result = ingest_mod.ingest("mock.olm", ingest_mod.IngestOptions(dry_run=True, batch_size=7))
    assert result == {"emails_parsed": 3}
    mock_impl.assert_called_once()


def test_reingest_bodies_wrapper_delegates_to_reingest_module() -> None:
    with patch("mailarium.ingest_reingest.reingest_bodies_impl", return_value={"message": "ok"}) as mock_impl:
...
def test_reset_index_wrapper_delegates_to_reingest_module() -> None:
    args = object()
    with patch("mailarium.ingest_reingest.reset_index_impl") as mock_impl:
...
def test_embed_pipeline_is_reexported_from_extracted_module() -> None:
    from mailarium.ingest_embed_pipeline import _EmbedPipeline as extracted_pipeline
...
def test_embed_pipeline_sentinel_is_reexported_from_extracted_module() -> None:
    from mailarium.ingest_embed_pipeline import _SENTINEL as extracted_sentinel
