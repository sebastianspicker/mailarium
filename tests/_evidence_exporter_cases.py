"""Semantic cases for public evidence-exporter test wrappers."""
...
def assert_html_export_contains_items() -> None:
    db = _seed_db()
...
def assert_export_file_rejects_existing_html_output_path(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_export_file_rejects_existing_csv_output_path(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_contains_quotes() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_respects_min_relevance() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_respects_category() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_verification_banner() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_appendix_contains_body() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_html_export_empty() -> None:
    db = EmailDatabase(":memory:")
    exporter = EvidenceExporter(db)
...
def assert_csv_export_headers() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_csv_export_data_rows() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_csv_export_respects_filters() -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_csv_export_neutralizes_formula_values_in_every_untrusted_column(monkeypatch) -> None:
...
def assert_export_file_html(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_export_file_csv(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_export_file_pdf_fallback(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_export_file_creates_parent_dirs(tmp_path: Path) -> None:
    db = _seed_db()
    exporter = EvidenceExporter(db)
...
def assert_evidence_stats_unfiltered() -> None:
    db = _seed_db()
...
def assert_evidence_stats_with_category_filter() -> None:
    db = _seed_db()
...
def assert_evidence_stats_with_min_relevance_filter() -> None:
    db = _seed_db()
...
def assert_evidence_stats_with_both_filters() -> None:
    db = _seed_db()
