...
from mailarium import cli_commands


def test_admin_wrapper_delegates_to_runtime_module() -> None:
    args = object()
    retriever = object()
    with patch("mailarium.cli_commands.runtime_family.cmd_admin_impl") as mock_impl:
        cli_commands._cmd_admin(args, retriever)
    mock_impl.assert_called_once_with(args, retriever)


def test_get_email_db_wrapper_delegates_to_runtime_module() -> None:
    with patch("mailarium.cli_commands.runtime_family.get_email_db_impl", return_value="db") as mock_impl:
        assert cli_commands._get_email_db() == "db"
    mock_impl.assert_called_once()


def test_analytics_wrapper_delegates_to_runtime_module() -> None:
    args = object()
    with patch("mailarium.cli_commands.runtime_family.run_analytics_command_impl") as mock_impl:
        cli_commands._run_analytics_command(args)
    mock_impl.assert_called_once()


def test_export_wrapper_delegates_to_family_module() -> None:
    with patch("mailarium.cli_commands_export.run_export_thread_impl") as mock_impl:
...
def test_search_wrapper_delegates_to_family_module() -> None:
    retriever = object()
    with patch("mailarium.cli_commands_search.run_single_query_impl", return_value=7) as mock_impl:
...
def test_browse_wrapper_delegates_to_family_module() -> None:
    with patch("mailarium.cli_commands_search.run_browse_impl") as mock_impl:
...
def test_evidence_wrapper_delegates_to_family_module() -> None:
    with patch("mailarium.cli_commands_evidence.run_evidence_stats_impl") as mock_impl:
...
def test_analytics_wrapper_delegates_to_family_module() -> None:
    with patch("mailarium.cli_commands_analytics.run_suggest_impl") as mock_impl:
...
def test_training_wrapper_delegates_to_family_module() -> None:
    with patch("mailarium.cli_commands_training.run_fine_tune_impl") as mock_impl:
        cli_commands._run_fine_tune("data.jsonl", "models/out", 4)
    mock_impl.assert_called_once_with("data.jsonl", "models/out", 4, mode="dense")
