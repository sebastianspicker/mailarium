...
def _is_forward_separator(line: str) -> bool:
    ...
    return (
        leading >= 3
        and trailing >= 3
        and stripped[leading : len(stripped) - trailing].strip().casefold() in _FORWARD_SEPARATOR_LABELS
    )


def _is_wrote_line(line: str) -> bool:
    normalized = line.rstrip().removesuffix(":").rstrip().casefold()
    return any(normalized.startswith(prefix) and marker in normalized[len(prefix) :] for prefix, marker in _WROTE_MARKERS)
