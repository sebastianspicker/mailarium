...
from src._utils import as_dict
...
class CitationRequest:
    """Typed values used to build one stable citation."""
...
def _base_citation(candidate: dict[str, Any], request: CitationRequest) -> dict[str, Any]:
    """Build one stable evidence citation."""
    speaker_status, quote_attribution_status = _speaker_attribution(request)
    provenance = as_dict(candidate.get("provenance"))
    attributed_status = "authored"
    if request.text_origin == "quoted":
        attributed_status = (
            "inferred" if quote_attribution_status in {"inferred_single_candidate", "participant_exclusion"} else "quoted"
        )
    return {
        "citation_id": (
            f"{request.finding_id}:{request.evidence_role}:{request.evidence_handle}:"
            f"{request.segment_ordinal or 0}:{request.start or 0}:{request.end or 0}"
        ),
        ...
        "actors": _actor_block(candidate, quoted_speaker=request.quoted_speaker),
        "text_attribution": {
            "text_origin": request.text_origin,
        ...
        "provenance": {
            ...
            "inference_basis": request.inference_basis,
            "evidence_chain_role": request.evidence_chain_role,
        },
        "note": request.note,
    }


def _speaker_attribution(request: CitationRequest) -> tuple[str, str]:
    if request.text_origin != "quoted":
        return "not_applicable", ""
    quoted = request.quoted_speaker or {}
    source = str(quoted.get("speaker_source") or "")
    status = str(quoted.get("quote_attribution_status") or "")
    if not source or source == "unresolved":
        return "unresolved", status
    return ("canonical" if source == "canonical_sender" else "inferred"), status
...
def _authored_citations(
    *,
    ...
    for index, evidence in enumerate(evidence_items, start=1):
        citations.append(_authored_citation(candidate, finding_id, evidence_handle, index, evidence))
    if citations:
        return citations
    return [
        _base_citation(
...
def _authored_citation(
    candidate: dict[str, Any], finding_id: str, evidence_handle: str, index: int, evidence: dict[str, Any]
) -> dict[str, Any]:
    metadata = str(evidence.get("source_scope") or "") == "message_metadata"
    return _base_citation(
        candidate,
        CitationRequest(
            finding_id=finding_id,
            evidence_role="supporting",
            excerpt=str(evidence.get("excerpt") or ""),
            text_origin="metadata" if metadata else "authored",
            ...
            provenance_kind="message_metadata" if metadata else "direct_text",
            inference_basis="message_local_metadata" if metadata else "message_local_text",
        ),
    )
...
def _quoted_citations(
    *,
    ...
    citations = _quoted_evidence_citations(
        candidate, finding_id, base_handle, segment_ordinal, quoted_block, quoted_speaker, evidence_items
    )
    if not citations:
        citations = [_fallback_quoted_citation(candidate, finding_id, base_handle, segment_ordinal, quoted_block, quoted_speaker)]
    downgraded = bool(quoted_block.get("downgraded_due_to_quote_ambiguity", True))
    reason = _quote_quality_reason(quoted_block, downgraded)
    return citations, {
...
def _quoted_evidence_citations(
    candidate: dict[str, Any],
    finding_id: str,
    base_handle: str,
    segment_ordinal: int,
    quoted_block: dict[str, Any],
    quoted_speaker: dict[str, Any],
    evidence_items: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    return [
        _base_citation(
...
def _quote_quality_reason(quoted_block: dict[str, Any], downgraded: bool) -> str:
    reason = str(quoted_block.get("quote_attribution_reason") or "")
    if downgraded and not reason:
        return "Quoted-speaker attribution is inferred or unresolved, so the finding should be read more cautiously."
    return reason
