...
def _normalized_roots(default_roots: tuple[str, ...], *, env_var: str) -> tuple[Path, ...]:
    ...
    relative = [item for item in configured if not Path(item).expanduser().is_absolute()]
    if relative:
        raise ValueError(f"{env_var} must contain only absolute paths")
...
def allowed_runtime_roots() -> tuple[Path, ...]:
    ...
    if os.getenv(_RUNTIME_HOME_ENV) or not _is_repository_checkout():
        ...
        relative = [item for item in configured if not Path(item).expanduser().is_absolute()]
        if relative:
            raise ValueError("MAILARIUM_ALLOWED_RUNTIME_ROOTS must contain only absolute paths")
