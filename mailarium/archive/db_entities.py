"""Entity repository collaborator for the canonical archive database."""
...
_ENTITY_MENTION_INCREMENT_SQL = """INSERT INTO entity_mentions(
...
_ENTITY_MENTION_IDEMPOTENT_SQL = """INSERT INTO entity_mentions(
       entity_id, email_uid, mention_count, extractor_key, extraction_version, extracted_at
   )
   VALUES(?, ?, 1, ?, ?, datetime('now'))
   ON CONFLICT(entity_id, email_uid) DO UPDATE SET
...
def _safe_optional_int(value: Any) -> int | None:
    """Parse an optional integer without propagating malformed stored values."""
    if value is None:
        return None
    try:
...
class _EntityOccurrence:
    """Represent one extracted entity occurrence before database persistence."""
...
def _entity_occurrence(row: tuple[object, ...]) -> _EntityOccurrence:
    """Normalize an extracted occurrence before entity persistence."""
    if len(row) != 9:
...
def _upsert_occurrence_entity(cur: sqlite3.Cursor, occurrence: _EntityOccurrence) -> int | None:
    """Upsert occurrence entity while preserving the invariants of entity database persistence."""
    row = cur.execute(
...
def _occurrence_hash(entity_id: int, email_uid: str, occurrence: _EntityOccurrence) -> str:
    """Hash occurrence identity fields to make repeated persistence idempotent."""
    values = (
        entity_id,
        email_uid,
...
def _persist_entity_occurrence(
    ...
    """Persist entity occurrence while preserving the invariants of entity database persistence."""
    cur.execute(
        """
...
class EntityRepository:
    ...
    def insert_entities_batch(
        ...
        *,
        ...
        self._insert_entity_mentions(
            ...
            mention_sql=_ENTITY_MENTION_INCREMENT_SQL,
            commit=commit,
        )
    ...
    def insert_entities_batch_idempotent(
        ...
        *,
        ...
        self._insert_entity_mentions(
            ...
            extractor_key=extractor_key,
            extraction_version=extraction_version,
            mention_sql=_ENTITY_MENTION_IDEMPOTENT_SQL,
            commit=commit,
        )
    ...
    def _insert_entity_mentions(
        ...
        *,
        ...
        """Upsert entities and persist mentions with the selected count policy."""
