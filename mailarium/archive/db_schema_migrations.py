...
from .mailbox_schema import MAILBOX_SCHEMA_SQL, ensure_mailbox_schema_compatibility
...
def apply_pending_migrations_impl(
    ...
    *,
    ...
    for version, migration in _migration_steps(cur, table_columns, sparse_schema_sql, vector_schema_sql):
        if current < version:
            migration()
...
def _migration_steps(
    ...
    """Return ordered schema migrations from the current version to the target version."""
    ...
    return (
        ...
        (35, lambda: _migrate_to_v35(cur, table_columns=table_columns, vector_schema_sql=vector_schema_sql)),
        (36, lambda: _migrate_to_v36(cur)),
    )
...
def _migrate_to_v35(
    ...
    *,
    ...
    """Add canonical dense-vector persistence and sparse model provenance."""
    cur.executescript(vector_schema_sql)
    existing = table_columns(cur, "sparse_vectors")
    _add_columns_safe(
        cur,
        "sparse_vectors",
        {
    ...
    logger.info("Schema migration v35: added SQLite-authoritative vector storage")


def _migrate_to_v36(cur: sqlite3.Cursor) -> None:
    """Add canonical mailbox source, synchronization, and action state."""
    cur.executescript(MAILBOX_SCHEMA_SQL)
    ensure_mailbox_schema_compatibility(cur)
    logger.info("Schema migration v36: added canonical mailbox state")
