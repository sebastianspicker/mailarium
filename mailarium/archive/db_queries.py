"""Message-query repository collaborator for the canonical archive database."""
...
class ArchiveQueryRepository:
    ...
    def _recipients_for_uid(self, uid: str) -> dict[str, list[str]]:
        """Return recipient fields for one message without loading its full body."""
        return recipients_for_uid_impl(self, uid)

    def _recipients_for_uids(self, uids: list[str]) -> dict[str, dict[str, list[str]]]:
        """Return recipient fields keyed by message UID for a batch lookup."""
    ...
    def get_thread_emails(self, conversation_id: str) -> list[dict]:
        """Return messages that share an explicit conversation identifier."""
        return get_thread_emails_impl(self, conversation_id)

    def get_inferred_thread_emails(self, inferred_thread_id: str) -> list[dict]:
        """Return messages grouped by the inferred thread identifier."""
    ...
    def consistency_check(self, vector_uids: set[str]) -> dict:
        """Compare SQLite email UIDs against USearch vector index chunk UIDs.

        USearch vector index chunk IDs follow the pattern ``{uid}__{chunk_index}``, so
        the email UID is the prefix before ``__``.

        Args:
            vector_uids: Set of chunk IDs from USearch vector index (e.g. via
                ``iter_vector_ids()``).

        Returns:
            Dict with ``sqlite_only`` (UIDs in SQLite but no chunks in
            USearch vector index), ``vector_only`` (UIDs with chunks in USearch vector index but
            missing from SQLite), and counts.
        """
        sqlite_uids = self.all_uids()

        # Extract email UIDs from chunk IDs: "uid__0" -> "uid"
        vector_email_uids: set[str] = set()
        for chunk_id in vector_uids:
            parts = chunk_id.split("__", 1)
            if parts:
                vector_email_uids.add(parts[0])

        sqlite_only = sqlite_uids - vector_email_uids
        vector_only = vector_email_uids - sqlite_uids

        return {
            "sqlite_count": len(sqlite_uids),
            "vector_uid_count": len(vector_email_uids),
            ...
            "is_consistent": len(sqlite_only) == 0 and len(vector_only) == 0,
        }
