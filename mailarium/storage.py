...
class SQLiteVectorCollection:
    ...
    def __init__(
        ...
        *,
        ...
        """Configure lazy SQLite storage and its rebuildable vector accelerator.
        ...
        self._owned_database: Any | None = None
    ...
    def attach_database(self, database: Any) -> None:
        """Use an existing EmailDatabase connection for transaction-safe writes."""
        with self._lock:
    ...
    def close(self) -> None:
        """Close a lazily created database connection."""
        with self._lock:
            if self._owned_database is not None:
                self._owned_database.close()
                self._owned_database = None
    ...
    def _connection(self) -> sqlite3.Connection:
        """Reuse an attached email database or lazily open an owned connection."""
        if self._database is not None:
            return self._database.conn
        if self._owned_database is None:
            from .email_db import EmailDatabase

            self._owned_database = EmailDatabase(self.sqlite_path)
        return self._owned_database.conn

    @contextmanager
    def _database_operation(self) -> Iterator[None]:
        """Cooperate with an attached EmailDatabase operation lock when present."""
        operation = getattr(self._database, "operation", None)
