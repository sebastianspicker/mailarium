...
def render_dashboard_page_impl(*, st_module: Any, database: Any | None) -> None:
    ...
    if email_input:
        contacts = database.top_contacts(email_input, limit=15)
        if contacts:
...
def render_entity_page_impl(*, st_module: Any, database: Any | None) -> None:
    ...
    entities = database.top_entities(entity_type=entity_type, limit=30)
    if entities:
    ...
    if entity_query:
        co_entities = database.entity_co_occurrences(entity_query, limit=20)
        if co_entities:
...
def render_network_page_impl(*, st_module: Any, database: Any | None) -> None:
