...
class EmailThreadLookupInput(StrictInput):
    ...
    def exactly_one_key(self):
        """Require one unambiguous thread lookup key before dispatching the request."""
...
class EmailReportInput(StrictInput):
    ...
    privacy_mode: Literal[
        ...
        default="full_access",
        description=(
            "Archive-report privacy mode. Redacted modes progressively suppress direct contact data, "
            "sensitive detail, and structured identities in the rendered report."
        ),
    )
    ...
    def validate_output_path(cls, v: str | None) -> str | None:
        """Reject report destinations outside the configured local output boundary."""
