...
from mailarium.model.search_scope import MAX_SCOPE_LENGTH, normalize_scope
...
class EmailSearchStructuredInput(DateRangeInput, MessageSearchFilterInput, StrictInput):
    ...
    scope: str | None = Field(
        ...
        description=(
            "Optional relevance scope such as 'general', 'finance', or 'research'. "
...
class FindSimilarInput(StrictInput):
    ...
    scope: str | None = Field(
        default=None,
        max_length=MAX_SCOPE_LENGTH,
        description="Optional per-request relevance scope for the similarity search.",
    )
    ...
    def at_least_one_input(self):
        """Require either a known email identifier or text to search around."""
...
class EmailTriageInput(DateRangeInput, StrictInput):
    ...
    scope: str | None = Field(
        default=None,
        max_length=MAX_SCOPE_LENGTH,
        description="Optional per-request relevance scope; hybrid weights are chosen automatically.",
    )
...
class EmailIngestInput(StrictInput):
    ...
    embed_images: bool = Field(
        default=False,
        description=(
            "If true, embed image attachments (JPG, PNG, etc.) into the separate "
            "SigLIP2 image-search space. Automatically enables extract_attachments."
        ),
    )
    ...
    vector_index_path: str | None = Field(
        default=None,
        description="Optional custom USearch vector index directory for the ingest run.",
    )
    ...
    def validate_olm_path(cls, value: str) -> str:
        """Require a readable local file whose extension identifies an OLM archive."""
    ...
    @field_validator("vector_index_path")
    @classmethod
    def validate_vector_index_path(cls, value: str | None) -> str | None:
        """Resolve optional vector-index overrides within the permitted runtime boundary."""
        return _validate_runtime_path(value, field_name="vector_index_path")

    @field_validator("sqlite_path")
    @classmethod
    def validate_sqlite_path(cls, value: str | None) -> str | None:
        """Resolve optional SQLite overrides within the permitted runtime boundary."""
...
class EmailExportInput(StrictInput):
    ...
    def validate_output_path(cls, v: str | None) -> str | None:
        """Reject export destinations outside the configured local output boundary."""
        return _validate_output_path(v)

    @model_validator(mode="after")
    def exactly_one_id(self):
        """Require one export target and forbid in-memory PDF output."""
...
class EmailScanInput(StrictInput):
    ...
    scan_id: str = Field(
        ...,
        description="Scan session identifier (e.g., 'project_review_2026', 'migration_notes').",
        min_length=1,
        max_length=100,
    )
    ...
    label: str | None = Field(
        default=None,
        description="Label for flagging or filtering candidates (e.g., 'decision', 'relevant', 'maybe').",
    )
