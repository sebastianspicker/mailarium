...
def _archive_texts(db: ArchiveDatabase) -> tuple[list[str], list[str]]:
...
def _build_topics(db: ArchiveDatabase, uids: list[str], texts: list[str], *, n_topics: int) -> None:
    """Fit NMF topic model and persist topics + email-topic assignments."""
    from mailarium.investigation.topic_modeler import TopicModeler
...
def _build_clusters(db: ArchiveDatabase, uids: list[str], texts: list[str], *, n_clusters: int | None) -> None:
