...
def render_interactive_intro_impl(console, panel_cls, retriever: SearchEngine) -> None:
...
def render_stats_impl(console, retriever: SearchEngine) -> None:
...
def _archive_summary(retriever: SearchEngine) -> tuple[dict[str, Any], str]:
...
def render_senders_impl(console, retriever: SearchEngine, *, print_sender_lines) -> None:
...
def run_suggest_impl(db) -> None:
    ...
    analytics_family.run_suggest_impl(db)


def run_generate_report_impl(db, output_path: str) -> None:
    ...
    export_family.run_generate_report_impl(db, output_path)


def run_export_thread_impl(db, conversation_id: str, fmt: str, output_path: str | None) -> None:
    ...
    export_family.run_export_thread_impl(db, conversation_id, fmt, output_path)


def run_export_email_impl(db, uid: str, fmt: str, output_path: str | None) -> None:
    ...
    export_family.run_export_email_impl(db, uid, fmt, output_path)


def run_browse_impl(db, offset: int, limit: int, folder: str | None, sender: str | None) -> None:
...
def run_evidence_list_impl(db, print_rich_or_plain, category: str | None, min_relevance: int | None) -> None:
    ...
    evidence_family.run_evidence_list_impl(db, print_rich_or_plain, category, min_relevance)
...
def run_evidence_export_impl(
    ...
    evidence_family.run_evidence_export_impl(db, output_path, fmt, category, min_relevance)


def run_evidence_stats_impl(db, print_rich_or_plain) -> None:
    ...
    evidence_family.run_evidence_stats_impl(db, print_rich_or_plain)


def run_evidence_verify_impl(db) -> None:
    ...
    evidence_family.run_evidence_verify_impl(db)


def run_dossier_impl(db, output_path: str, fmt: str, category: str | None, min_relevance: int | None) -> None:
    ...
    evidence_family.run_dossier_impl(db, output_path, fmt, category, min_relevance)


def run_custody_chain_impl(db, print_rich_or_plain) -> None:
    ...
    evidence_family.run_custody_chain_impl(db, print_rich_or_plain)


def run_provenance_impl(db, email_uid: str) -> None:
    ...
    evidence_family.run_provenance_impl(db, email_uid)


def run_export_network_impl(db, output_path: str) -> None:
    ...
    export_family.run_export_network_impl(db, output_path)


def run_generate_training_data_impl(db, output_path: str) -> None:
    ...
    training_family.run_generate_training_data_impl(db, output_path)
