...
class CommunicationNetwork:
    ...
    def __init__(self, email_db: ArchiveDatabase) -> None:
        self._db = email_db
    ...
    def network_analysis(self, top_n: int = 20) -> dict[str, Any]:
        ...
        cache_key = (graph.number_of_nodes(), graph.number_of_edges(), top_n)
    ...
    def _get_betweenness(self, nx) -> dict[str, float]:
        ...
        if graph is None:
            return {}
        cache_key = (graph.number_of_nodes(), graph.number_of_edges())
    ...
    def _path_edges(self, nodes):
        ...
        for a, b in pairwise(nodes):
            weight = graph[a][b].get("weight", 0) if graph.has_edge(a, b) else 0
            weight += graph[b][a].get("weight", 0) if graph.has_edge(b, a) else 0
    ...
    def shared_recipients(self, email_addresses: list[str], min_shared: int = 2) -> list[dict[str, Any]]:
        ...
        Delegates to ArchiveDatabase SQL query for accuracy.
    ...
    def relationship_summary(self, email_address: str, limit: int = 20) -> dict[str, Any]:
        ...
        if nx and graph is not None and graph.number_of_nodes() > 0 and email_address in graph:
            ...
            send_count = sum(data.get("weight", 0) for _, _, data in graph.out_edges(email_address, data=True))
            receive_count = sum(data.get("weight", 0) for _, _, data in graph.in_edges(email_address, data=True))
    ...
    def export_graphml(self, output_path: str) -> dict[str, Any]:
        ...
        graph = self._graph
        if graph is None:
            return {"error": "communication graph is unavailable"}
