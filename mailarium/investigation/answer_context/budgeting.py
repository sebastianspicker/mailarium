"""Deterministic response-size budgeting helpers."""
...
def _attachment_strength_score(item: dict[str, Any]) -> int:
    """Score attachment evidence from zero for weak content to three for verified strong text."""
...
def _anchor_timeline_events(events: list[dict[str, Any]], wanted: list[str]) -> tuple[list[dict[str, Any]], set[str]]:
    """Keep requested timeline UIDs first, once each, while tracking identities already selected."""
    kept: list[dict[str, Any]] = []
    seen: set[str] = set()
...
def _fill_timeline_events(events: list[dict[str, Any]], kept: list[dict[str, Any]], seen: set[str], max_events: int) -> None:
    """Fill remaining timeline capacity in source order without duplicating anchored UIDs."""
    for event in events:
...
def _compact_conversation_group(group: dict[str, Any]) -> dict[str, Any]:
    """Compact conversation group to keep bounded tool responses useful."""
    return {
...
def _pop_fields(item: dict[str, Any], fields: list[str]) -> int:
    """Remove optional payload fields in place and report how many keys changed."""
    changes = 0
    for field in fields:
        if field in item:
            item.pop(field, None)
...
def _compact_attachment_field(candidate: dict[str, Any]) -> int:
    """Compact attachment field to keep bounded tool responses useful."""
...
def _strip_optional_evidence_fields(
    ...
    for candidate in candidates:
        changes += _pop_fields(
            candidate,
            ["body_render_mode", "body_render_source", "verification_status", "speaker_attribution"],
        )
