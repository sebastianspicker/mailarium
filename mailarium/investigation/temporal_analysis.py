...
def _resolve_zoneinfo_name(candidate: str) -> str | None:
    """Validate a candidate IANA timezone name after removing an optional TZ prefix colon."""
...
def _local_display_timezone() -> tzinfo:
    """Prefer the system IANA zone and fall back to the active local offset or UTC."""
...
def _response_time_results(sender_times: dict[str, list[float]], *, pair_limit: int, limit: int) -> list[dict[str, Any]]:
    ...
    result.sort(key=lambda item: int(item["response_count"]), reverse=True)
    return result[:limit]
...
class TemporalAnalyzer:
    ...
    def __init__(self, email_db: ArchiveDatabase, *, display_timezone: str | tzinfo | None = None) -> None:
        """Bind the analytics database and timezone used when rendering time-based summaries."""
        self._db = email_db
        self._display_timezone = _resolve_display_timezone(display_timezone)

    def _normalized_dates(self, dates: list[str]):
        """Parse timestamps as UTC, convert to the display timezone, and drop invalid values."""
        import pandas as pd
    ...
    def activity_heatmap(self) -> list[dict[str, Any]]:
