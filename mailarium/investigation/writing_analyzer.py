...
class WritingMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize writing metrics with stable rounding for display and export."""
        return {
...
class WritingAnalyzer:
    ...
    def _readability(self, text: str, word_count: int) -> tuple[float | None, float | None]:
        """Compute Flesch metrics for sufficiently long text, returning no scores on optional-library failure."""
...
def _safe_avg(values: list[float]) -> float | None:
    """Return a rounded mean or no value for an empty metric sample."""
    if not values:
        return None
