...
def _canonical_thread_graph(email: dict[str, Any]) -> dict[str, Any]:
    """Expose canonical conversation, reply, and reference links with an explicit link-presence flag."""
...
def _email_references(email: dict[str, Any]) -> list[Any]:
    """Decode stored reference JSON defensively and return only list-shaped message references."""
...
def _inferred_thread_graph(email: dict[str, Any]) -> dict[str, Any]:
    """Expose inferred parent, thread, reason, and confidence fields with an explicit link-presence flag."""
...
def _estimated_json_chars(payload: dict[str, Any]) -> int:
    """Estimate compact UTF-8 JSON character size for response-budget decisions."""
    return len(json.dumps(payload, ensure_ascii=False, default=str, separators=(",", ":")))


def _trim_text(value: Any, max_chars: int) -> str:
    """Truncate text to the requested limit while reserving room for an ellipsis."""
...
def _compact_email_body_metadata(email: Any) -> None:
    """Compact email body metadata to keep bounded tool responses useful."""
...
def _compact_conversation_debug(debug: Any) -> None:
    """Compact conversation debug to keep bounded tool responses useful."""
...
def _compact_thread_payload(thread: Any) -> None:
    """Compact thread payload to keep bounded tool responses useful."""
...
def _compact_evidence_payload(evidence: Any) -> None:
    """Compact evidence payload to keep bounded tool responses useful."""
...
def register(mcp: Any, deps: ToolDepsProto) -> None:
    ...
    _register_export_tool(mcp, deps)
    _register_browse_tool(mcp, deps)
    _register_deep_context_tool(mcp, deps)


def _register_export_tool(mcp: Any, deps: ToolDepsProto) -> None:
    """Register the export tool without coupling it to sibling tool modules."""
...
def _register_browse_tool(mcp: Any, deps: ToolDepsProto) -> None:
    """Register the browse tool without coupling it to sibling tool modules."""
...
def _register_deep_context_tool(mcp: Any, deps: ToolDepsProto) -> None:
    # email_get_full removed - subsumed by email_deep_context(include_thread=False, ...)
    """Register the deep context tool without coupling it to sibling tool modules."""
...
def _thread_summary_email(email: dict, deps) -> dict[str, str]:
    """Sanitize body text and retain only fields consumed by thread summarization."""
    return {
...
def _thread_timeline_email(email: dict) -> dict[str, str]:
    """Reduce an email to sender, calendar date, and subject for timeline rendering."""
...
def _deep_evidence_item(item: dict) -> dict[str, Any]:
    """Limit deep-context evidence to public fields and an 80-character quote preview."""
