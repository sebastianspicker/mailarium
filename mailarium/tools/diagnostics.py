...
def _qa_eval_report_candidates() -> list[Path]:
    """Resolve public and private QA evaluation report locations in preference order."""
    return summary_family.qa_eval_report_candidates_impl(_repo_root)


def _qa_eval_remediation_candidates() -> list[Path]:
    """Resolve candidate QA remediation reports without hard-coding the repository root."""
    return summary_family.qa_eval_remediation_candidates_impl(_repo_root)


def _inferred_thread_prevalence_candidates() -> list[Path]:
    """Resolve candidate inferred-thread prevalence reports across supported result locations."""
...
def _load_eval_report(path: Path) -> tuple[str, dict[str, Any]] | None:
    """Load eval report while preserving the caller's fallback behavior."""
...
def _load_remediation_report(path: Path) -> tuple[str, dict[str, Any]] | None:
    """Load remediation report while preserving the caller's fallback behavior."""
...
def _load_inferred_thread_prevalence(path: Path) -> tuple[str, dict[str, Any]] | None:
    """Load inferred thread prevalence while preserving the caller's fallback behavior."""
...
def _scored_metric_rate(metric: dict[str, Any]) -> dict[str, Any]:
    """Normalize a scored metric with the shared safe-rate calculation."""
...
def _prefer_specialized_summary(
    *,
    ...
    """Select specialized summary using the tool's deterministic policy."""
...
def _answer_task_readiness_summary() -> dict[str, Any]:
    """Combine QA, remediation, and thread-prevalence reports into answer-task readiness evidence."""
