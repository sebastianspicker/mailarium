...
class EvidenceAddInput(StrictInput):
    ...
    category: str = Field(
        ...,
        description=(
            "User-defined category for this evidence item, such as fact, decision, commitment, contradiction, risk, or general."
        ),
        min_length=1,
        max_length=80,
    )
...
class EvidenceUpdateInput(StrictInput):
    ...
    category: str | None = Field(
        default=None,
        description="New user-defined evidence category.",
        min_length=1,
        max_length=80,
    )
    ...
    def require_update(self) -> EvidenceUpdateInput:
        """Reject no-op evidence updates that would otherwise appear successful."""
        if all(
            value is None
            for value in (
                self.category,
...
class EvidenceExportInput(StrictInput):
    ...
    def validate_output_path(cls, v: str) -> str:
        """Reject evidence-export destinations outside the configured local output boundary."""
...
class EmailDossierInput(StrictInput):
    ...
    title: str = Field(
        ...
        description="Title for the collection cover page.",
    )
    collection_reference: str = Field(
        default="",
        description="Optional project or collection reference.",
    )
    custodian: str = Field(default="", description="Name of the collection owner or evidence custodian.")
    ...
    @classmethod
    def validate_output_path(cls, v: str) -> str:
        """Reject dossier destinations outside the configured local output boundary."""
        return _validate_output_path(v)  # type: ignore[return-value]
