"""Pure attachment surface representation and persistence-row projection."""
...
class AttachmentSurfaceSpec:
    """Inputs used to normalize one attachment's searchable surfaces."""
...
def _surface_payload(
    *,
    ...
    """Create a normalized searchable surface record for one attachment representation."""
...
def build_attachment_surfaces(
    *,
    spec: AttachmentSurfaceSpec | None = None,
    **values: Any,
) -> list[dict[str, Any]]:
    """Return normalized attachment surfaces with stable defaults."""
    if spec is not None and values:
        raise TypeError("spec cannot be combined with attachment surface keyword values")
    resolved = spec or AttachmentSurfaceSpec(**values)
    locator = _dict(resolved.text_locator)
    normalized_surfaces: list[dict[str, Any]] = []
    if isinstance(resolved.surfaces, list):
            ...
            if isinstance(surface, dict):
                normalized_surfaces.append(
                    _normalized_surface(
                        ...
                        locator=locator,
                        extraction_state=resolved.extraction_state,
    ...
    return _default_surfaces(
        attachment_id=resolved.attachment_id,
        extracted_text=_text(resolved.extracted_text),
        normalized_text=_text(resolved.normalized_text),
        text_locator=locator,
        extraction_state=_text(resolved.extraction_state),
        evidence_strength=_text(resolved.evidence_strength),
        ocr_used=bool(resolved.ocr_used),
        ocr_confidence=float(resolved.ocr_confidence or 0.0),
    )
...
def _normalized_surface(
    ...
    *,
    ...
    """Merge extraction metadata into a deterministic searchable surface payload."""
...
def _surface_content(surface: dict[str, Any]) -> tuple[str, str, str, str]:
    """Return the searchable content from explicit text or decoded bytes."""
    return (
...
def _default_surface_text(value: Any, default: str) -> str:
    """Choose the default searchable text when no explicit surfaces exist."""
...
def _surface_locator(surface: dict[str, Any], default_locator: dict[str, Any]) -> dict[str, Any]:
    """Normalize a surface locator into a JSON-safe mapping."""
...
def _surface_quality(surface: dict[str, Any], state: str, strength: str, ocr_used: bool) -> dict[str, Any]:
    """Normalize the declared extraction quality for a surface."""
...
def _surface_ocr_confidence(surface: dict[str, Any], default: float) -> float:
    """Coerce optional OCR confidence to a bounded numeric value."""
...
def _apply_explicit_surface_identifiers(
    ...
    """Copy stable caller-provided identifiers onto a normalized surface."""
...
def _apply_surface_language(payload: dict[str, Any], surface: dict[str, Any]) -> None:
    """Attach a normalized language tag when one is available."""
