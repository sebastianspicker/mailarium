...
def cmd_mailbox(args: argparse.Namespace) -> None:
    """Dispatch the ``mailarium mailbox`` command family."""
    settings = get_settings()
    service = mailbox_service_for_path(
        getattr(args, "sqlite_path", None) or settings.sqlite_path,
        vector_index_path=getattr(args, "vector_index_path", None) or settings.vector_index_path,
    )
    try:
