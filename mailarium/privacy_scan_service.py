...
from mailarium.privacy_scan_git import history_blobs, history_paths, run_git_bytes, tracked_paths, untracked_paths
from mailarium.privacy_scan_rules import (
...
def _scan_text(root: Path, paths: list[str], *, index_fallback: bool = False) -> list[Finding]:
    """Scan eligible worktree files or index fallbacks without returning content."""
    ...
    for path in paths:
        ...
        try:
            ...
            if not index_fallback:
                continue
            text = run_git_bytes(root, ["show", f":{path}"], check=False).decode("utf-8", errors="ignore")
        except OSError:
...
def scan(root: Path, *, include_untracked: bool = True, include_history: bool = False) -> list[Finding]:
    ...
    findings.extend(_scan_text(root, tracked, index_fallback=True))
