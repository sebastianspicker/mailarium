...
class ScanSession:
    ...
    def touch(self) -> None:
        """Refresh the session access timestamp used for TTL expiry."""
        self.last_accessed = time.time()

    def is_expired(self, ttl: float) -> bool:
        """Return whether inactivity exceeds the configured session TTL."""
...
def _get_ttl() -> float:
    """Read a positive finite session TTL or fall back to one hour."""
...
def _cleanup_expired() -> None:
    """Evict inactive sessions before the shared registry is accessed."""
    ttl = _get_ttl()
...
def get_candidates(
    ...
    *,
    ...
    """Return flagged candidates matching optional label and phase filters under the session lock.
...
def session_status(scan_id: str) -> dict | None:
    """Summarize seen and flagged counts without racing concurrent session updates.
...
def reset_all_sessions() -> int:
    """Clear all in-memory sessions and return the number removed."""
    with _lock:
        count = len(_sessions)
        _sessions.clear()
        return count
