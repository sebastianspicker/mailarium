...
from mailarium.archive.visibility import has_tombstoned_mailbox_sources
...
from .retriever_models import SearchRequest, SearchResponse, SearchResult
...
from .visibility import visible_results
...
class SearchEngine:
    """Canonical local search facade over SQLite records and derived indexes.
    ...
    The facade owns their lifecycle and keeps legacy caller methods stable.
    """
    ...
    def _image_results_from_payload(payload: dict[str, Any]) -> list[SearchResult]:
        ...
        return [
            SearchEngine._image_result_from_payload_row(chunk_id, index, documents, metadatas, distances)
    ...
    def execute(self, request: SearchRequest) -> SearchResponse:
        ...
        if plan is None:
            return SearchResponse(results=(), diagnostics=copy.deepcopy(self.last_search_debug))
        results = self._execute_filtered_search(plan, filters)
        return SearchResponse(results=tuple(results), diagnostics=copy.deepcopy(self.last_search_debug))

    def _prepare_filtered_search(self, request: SearchRequest) -> tuple[_SearchPlan | None, _SearchFilters]:
    ...
    def _apply_rerank(self, query: str, results: list[SearchResult], top_k: int) -> list[SearchResult]:
        ...
        if use_late_interaction and settings is not None:
            try:
                if self._late_interaction_backend is None:
                    ...
                    self._late_interaction_backend = LocalLateInteractionBackend(
                        ...
                        runner_sha256=getattr(settings, "late_interaction_runner_sha256", None) or None,
                        allowed_runner_directory=getattr(settings, "late_interaction_runner_directory", None) or None,
                    )
    ...
    def _active_mailbox_results(self, results: list[SearchResult]) -> list[SearchResult]:
        ...
        return visible_results(self.email_db, results)
