...
class LocalLateInteractionBackend:
    ...
    allowed_runner_directory: str | None = None
    ...
    def rerank(
        ...
        *,
        ...
        if not results:
            return []
        runner = _validated_file(
            self.runner_path,
            label="late-interaction runner",
            expected_sha256=self.runner_sha256,
            allowed_directory=self.allowed_runner_directory,
        )
...
def _run_bounded_runner(
    ...
    *,
    ...
    """Run a validated local runner with bounded pipes and a scrubbed environment."""
    command = [str(runner)]
    ...
    with tempfile.TemporaryFile() as request_stream:
        ...
        process = subprocess.Popen(
            ...
            cwd=str(cwd),
            env=dict(_SAFE_RUNNER_ENVIRONMENT),
            close_fds=True,
        )
...
def _validated_file(
    ...
    *,
    ...
    """Validate a non-writable regular file at an explicitly allowed absolute path."""
    configured = Path(value)
    if not configured.is_absolute():
        raise LateInteractionError(f"{label} path must be absolute")
    path = configured.resolve()
    if not path.is_file():
        raise LateInteractionError(f"{label} does not exist: {path}")
    _reject_group_or_world_writable(path, label)
    if allowed_directory:
        allowed = Path(allowed_directory)
        if not allowed.is_absolute():
            raise LateInteractionError("late-interaction allowed runner directory must be absolute")
        allowed = allowed.resolve()
        if allowed not in path.parents:
            raise LateInteractionError(f"{label} is outside the configured allowed runner directory")
        _reject_group_or_world_writable(allowed, "late-interaction allowed runner directory")
    if expected_sha256 is not None:
        expected = expected_sha256.strip().lower()
        if len(expected) != 64 or any(char not in "0123456789abcdef" for char in expected):
            raise LateInteractionError("late-interaction runner SHA-256 must be a 64-character hexadecimal digest")
        if not hmac.compare_digest(_sha256(path), expected):
            raise LateInteractionError("late-interaction runner checksum mismatch")
    return path
...
def _validated_model(value: str) -> tuple[Path, dict[str, Any]]:
    ...
    _reject_group_or_world_writable(artifact, "late-interaction model artifact")
...
def _reject_group_or_world_writable(path: Path, label: str) -> None:
    """Reject mutable execution/model artifacts that another account could replace."""
    try:
        mode = path.stat().st_mode
    except OSError as exc:
        raise LateInteractionError(f"cannot inspect {label}: {exc}") from exc
    if mode & (stat.S_IWGRP | stat.S_IWOTH):
        raise LateInteractionError(f"{label} must not be group- or world-writable")
