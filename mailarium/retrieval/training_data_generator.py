...
Output format: provider-neutral JSONL with ``query``, ``pos``, and ``neg``.
"""
...
class TrainingDataGenerator:
    ...
    def __init__(self, db: ArchiveDatabase, seed: int = 42) -> None:
        """Bind the source database and deterministic random generator for training examples."""
        self._db = db
    ...
    def _build_triplet(
        ...
        """Create a non-empty query-positive pair and attach a hard or random cross-thread negative."""
    ...
    def _load_email_data(self, min_thread_size: int) -> tuple[dict[str, list[dict]], dict[str, list[dict]], list[dict]]:
        """Scan emails once to build eligible threads, sender groups, and the full negative pool.
...
def _negative_candidates(emails: list[dict], conversation_id: str) -> list[dict]:
    """Keep non-empty emails from conversations other than the query thread."""
