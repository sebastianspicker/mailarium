...
from mailarium.ingestion.olm.xml_helpers import _parse_references
from mailarium.model.conversation_segments import ConversationSegment, extract_segments
...
class ParsedEmailParts:
    """Hold normalized message identity and header values before ParsedMessage construction."""
...
class ParsedEmailEnrichments:
    """Hold derived body, classification, and reply-context data for parsed mail."""
...
def _fill_source_basics(parts: ParsedEmailParts) -> None:
    """Fill missing sender, subject, and date fields from source-message headers."""
...
def _fill_source_to_addresses(parts: ParsedEmailParts, extract_identity_addresses_fn: Any) -> None:
    """Recover primary recipients only when parsed XML addresses are absent."""
...
def _fill_source_cc_addresses(parts: ParsedEmailParts, extract_identity_addresses_fn: Any) -> None:
    """Recover carbon-copy recipients without replacing parsed XML addresses."""
...
def _fill_source_bcc_addresses(parts: ParsedEmailParts, extract_identity_addresses_fn: Any) -> None:
    """Recover blind-copy recipients without replacing parsed XML addresses."""
...
def _fill_source_thread_headers(parts: ParsedEmailParts) -> None:
    """Backfill reply and reference headers needed for canonical thread linkage."""
...
def _fill_recipient_identities(parts: ParsedEmailParts, extract_identity_addresses_fn: Any) -> None:
    """Derive normalized recipient identities after address fallbacks are complete."""
...
def _fill_body_fallback(parts: ParsedEmailParts) -> None:
    """Recover a visible body from source text when structured bodies are empty."""
...
def _normalize_part_headers(parts: ParsedEmailParts) -> None:
    """Decode MIME headers and normalize addresses before ParsedMessage construction."""
...
def derive_email_enrichments(
    ...
    *,
    ...
    """Derive forensic text, body normalization, and message classification for parsed XML fields."""
