...
from mailarium.model.attachment_profiles import IMAGE_EXTENSIONS as _IMAGE_EXTENSIONS
from mailarium.model.rfc2822 import _extract_body_from_source, _extract_header
...
def _extract_standard_attachment(
    ...
    *,
    ...
    """Run the selected text extractor and normalize its success or failure result."""
...
def _default_extractors() -> dict[str, Callable[..., str | None]]:
    """Map supported filename extensions to their built-in text extractors."""
    return {
...
def _eml_headers(raw_source: str) -> dict[str, str]:
    """Render selected RFC message headers as searchable text."""
    return {name: _extract_header(raw_source, name) for name in ("Subject", "From", "Date")}


def _eml_message(content: bytes):
    """Parse an EML payload and combine headers, body, and attachment text."""
    try:
...
def _eml_body(raw_source: str) -> str:
    """Extract the preferred readable body from a parsed email message."""
...
def _eml_parts(headers: dict[str, str], message: Any, body_text: str) -> list[str]:
    """Walk MIME parts and collect readable body and attachment content."""
