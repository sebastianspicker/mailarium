...
def _attachment_completion_status(email: ParsedMessage) -> str:
...
def _image_completion_status(email: ParsedMessage) -> str:
...
def _ingest_state_rows(emails: list[ParsedMessage]) -> list[dict[str, object]]:
...
class _EmbedPipeline:
    ...
    def submit(self, chunks: list[EmailChunk], emails: list[ParsedMessage]) -> None:
    ...
    def _log_deduplicated_batch(
        emails: list[ParsedMessage], chunks: list[EmailChunk], new_emails: list[ParsedMessage], new_chunks: list[EmailChunk]
    ) -> None:
    ...
    def _persist_batch_metadata(self, emails: list[ParsedMessage]) -> tuple[float, float]:
    ...
    @staticmethod
    def _has_entity_surface(email: ParsedMessage) -> bool:
    ...
    def _persist_entity_occurrences(
        self,
        email: ParsedMessage,
    ...
    def _persist_exchange_entities(self, emails: list[ParsedMessage]) -> None:
...
def _exchange_entities_from_email(email: ParsedMessage) -> list[tuple[str, str, str]]:
    """Extract entity tuples from Exchange-extracted fields on a ParsedMessage object."""
    entities: list[tuple[str, str, str]] = []
