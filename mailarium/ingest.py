...
class _IngestOptionOverrides(TypedDict, total=False):
    """Keyword-compatible ingestion options for the legacy call form."""
...
class IngestOptions:
    """Group optional ingestion controls behind one typed request object."""
...
class _NoOpProgressBar:
    ...
    def update(self, n: int = 1) -> None:
        """Accept progress updates when the optional progress dependency is absent."""

    def close(self) -> None:
        """Provide the progress-bar close protocol without side effects."""

    def set_postfix(self, **kwargs: Any) -> None:
        """Accept progress metadata when no concrete progress bar is installed."""
...
def ingest(
    ...
    **option_overrides: Unpack[_IngestOptionOverrides],
) -> dict[str, Any]:
    ...
    and stores results in USearch vector index and SQLite databases.

    Args:
        olm_path: Path to the Outlook .olm export file.
        options: Grouped ingestion controls. Existing keyword options remain
            accepted for compatibility and cannot be mixed with this object.

    Returns:
        Dictionary containing ingestion statistics (emails parsed, chunks created, etc.).
    """
    if options is not None and option_overrides:
        raise TypeError("ingest options cannot be mixed with keyword overrides")
    resolved = options or IngestOptions(**option_overrides)
    ...
    return pipeline_family.ingest_impl(
        ...
        max_emails=resolved.max_emails,
        dry_run=resolved.dry_run,
        extract_attachments=resolved.extract_attachments,
        extract_entities=resolved.extract_entities,
        incremental=resolved.incremental,
        embed_images=resolved.embed_images,
...
def _add_ingest_arguments(parser: argparse.ArgumentParser) -> None:
    """Register source, index, and batching options for the ingest command."""
...
def _add_reprocessing_arguments(parser: argparse.ArgumentParser) -> None:
    """Register selectors and extraction controls for reprocessing commands."""
    parser.add_argument(
        "--incremental",
        action="store_true",
...
def _add_common_arguments(parser: argparse.ArgumentParser) -> None:
    """Register database and logging options shared by ingest subcommands."""
...
def _run_maintenance_command(args: argparse.Namespace) -> bool:
    """Dispatch a requested maintenance action before normal ingestion begins."""
    if args.reset_index:
...
def _run_reset_index(args: argparse.Namespace) -> None:
    """Require explicit confirmation, reset the index, and terminate the CLI."""
    if not args.yes:
...
def _print_command_completion(result: dict[str, Any]) -> None:
    """Print a maintenance result message and terminate successfully."""
    print(result["message"])
    raise SystemExit(0)


def _run_ingest_command(args: argparse.Namespace) -> dict[str, Any]:
    """Translate validated CLI arguments into one ingestion request."""
    try:
        return ingest(
            args.olm_path,
            vector_index_path=args.vector_index_path,
...
def reembed(
    ...
    embeddings in USearch vector index.
...
def _reset_index(args: argparse.Namespace) -> None:
    """Clear rebuildable vector state while preserving email metadata."""
...
def format_ingestion_summary(stats: dict[str, Any]) -> list[str]:
    ...
    _append_write_summary(lines, stats)
...
def _append_write_summary(lines: list[str], stats: dict[str, Any]) -> None:
    """Append relational and vector write counts to the command summary."""
    if stats["dry_run"]:
...
def _append_timing_summary(lines: list[str], timing_info: Any) -> None:
    """Append measured ingest phase durations when detailed timing is enabled."""
    if not timing_info:
        return
    timing_parts = [
    ...
    detail_parts = [f"{label}={timing_info[key]}s" for key, label in _INGEST_TIMING_DETAIL_KEYS if key in timing_info]
    if detail_parts:
        lines.append(f"  Breakdown: {', '.join(detail_parts)}")


_INGEST_TIMING_DETAIL_KEYS = (
    ("parse_seconds", "parse"),
