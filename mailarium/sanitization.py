...
_FULL_ACCESS_RULES: dict[str, Any] = {
    "audience": "internal_review",
    "description": "Full review mode with no privacy redaction beyond terminal sanitization.",
    "redact_contact": False,
    "redact_privileged": False,
    "redact_medical": False,
    "redact_structured_identity": False,
}
_CONTACT_REDACTED_RULES: dict[str, Any] = {
    "audience": "controlled_external_review",
    "description": "Contact-redacted review that preserves the remaining source context.",
    "redact_contact": True,
    "redact_privileged": False,
    "redact_medical": False,
    "redact_structured_identity": False,
}
_SENSITIVE_REDACTED_RULES: dict[str, Any] = {
    "audience": "restricted_internal_review",
    "description": "Sensitive-data review that suppresses contact, privileged, and medical detail.",
    "redact_contact": True,
    "redact_privileged": True,
    "redact_medical": True,
    "redact_structured_identity": False,
}
_STRICT_REDACTION_RULES: dict[str, Any] = {
    "audience": "limited_circulation",
    "description": "Strict least-exposure review that also suppresses structured participant identities.",
    "redact_contact": True,
    "redact_privileged": True,
    "redact_medical": True,
...
PRIVACY_MODE_RULES: dict[str, dict[str, Any]] = {
    "full_access": _FULL_ACCESS_RULES,
    "contact_redacted": _CONTACT_REDACTED_RULES,
    "sensitive_redacted": _SENSITIVE_REDACTED_RULES,
    "strict_redaction": _STRICT_REDACTION_RULES,
}
...
def csv_safe_cell(value: Any) -> str:
    """Prefix formula-leading CSV values so spreadsheet software treats them as text.
...
def resolve_privacy_mode(mode: str | None) -> str:
    """Normalize a privacy-mode name and reject unsupported policies."""
...
def privacy_mode_policy(mode: str | None) -> dict[str, Any]:
    """Expose the audience and active redaction rules for a validated privacy mode."""
...
def apply_privacy_guardrails(payload: Any, *, privacy_mode: str | None) -> tuple[Any, dict[str, Any]]:
    """Recursively sanitize and redact a payload while reporting category-level redaction counts."""
