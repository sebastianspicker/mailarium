...
DEFAULT_QA_EVAL_FIXTURES_DIR = ROOT / "tests" / "fixtures" / "qa_eval"
...
class CapturedEvalScenario:
    ...
    def resolve(self, fixtures_dir: Path = DEFAULT_QA_EVAL_FIXTURES_DIR) -> ResolvedCapturedEvalScenario:
        """Bind this scenario's fixture filenames to a concrete fixtures directory."""
        return ResolvedCapturedEvalScenario(
            name=self.name,
            questions_path=fixtures_dir / self.questions_filename,
            results_path=fixtures_dir / self.results_filename,
            report_path=fixtures_dir / self.report_filename,
        )
...
def captured_eval_scenarios(fixtures_dir: Path = DEFAULT_QA_EVAL_FIXTURES_DIR) -> tuple[ResolvedCapturedEvalScenario, ...]:
    """Resolve every registered captured scenario against one fixture directory."""

    return tuple(scenario.resolve(fixtures_dir) for scenario in CAPTURED_EVAL_SCENARIOS)
...
def refresh_captured_eval_reports(
    *,
    fixtures_dir: Path = DEFAULT_QA_EVAL_FIXTURES_DIR,
    ...
    for scenario in captured_eval_scenarios(fixtures_dir):
