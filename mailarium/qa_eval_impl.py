...
class EvaluationRequest:
    """Internal input snapshot for one QA evaluation run."""
...
def load_eval_report(path: Path) -> dict[str, Any]:
    """Load a persisted evaluation report without altering its serialized structure."""
...
async def run_evaluation(
    ...
    """Evaluate requested cases from captured results, live retrieval, or their configured combination."""
    cases = load_question_cases(request.questions_path)
    if request.limit is not None:
        cases = cases[: request.limit]

    captured = _results_payload_map(request.results_path) if request.results_path else {}
    resolved_source_mode = _resolve_source_mode(
        results_path=request.results_path,
        live_deps=request.live_deps,
        source_mode=request.source_mode,
    )

    report: dict[str, Any] = {
        "questions_path": _display_path(request.questions_path),
        "results_path": _display_path(request.results_path),
        "total_cases": len(cases),
        "cases": [_serialize_case(case) for case in cases],
        "source_mode": resolved_source_mode,
        "live_backend": getattr(request.live_deps, "live_backend", None) if request.live_deps is not None else None,
    }

    if resolved_source_mode == "mixed":
        report.update(await _mixed_evaluation_report(cases, captured, request.live_deps))
